import jieba
import numpy as np
from .text_rank import TextRank_english
...
def extract_keywords(text):
    ...
    textrank_keyword = TextRank_english()
    textrank_keyword.build_graph(sentences)
    keywords = textrank_keyword.get_keywords(n=10)
    # print("關鍵詞:", keywords)


    # --- 提取關鍵句 ---
    sentences = [s.strip() for s in text.split('。') if s.strip()] # 分句，去除空句子
    textrank_sentence = TextRank_english()
    textrank_sentence.build_graph(sentences)
    key_sentences = textrank_sentence.get_key_sentences(n=5)
    # print("\n關鍵句:", key_sentences)
